...
echo "        try_files $uri =404;" >> $NGINX_DEF
echo "        fastcgi_index index.php;" >> $NGINX_DEF
echo "        include fastcgi_params;" >> $NGINX_DEF
echo "        fastcgi_pass unix:/run/php/php${PHP_VERSION}-fpm.sock;" >> $NGINX_DEF
echo "        fastcgi_param SCRIPT_FILENAME $document_root$fastcgi_script_name;" >> $NGINX_DEF
echo "    }" >> $NGINX_DEF
echo "}" >> $NGINX_DEF
